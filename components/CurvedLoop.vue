...
import { ref, onMounted, onUnmounted, watch, computed, nextTick, useId } from 'vue';
...
const animate = () => {
  if (!spacing.value || !ready.value || prefersReduced.value) return;
...
<template>
  ...
  >
    <svg
      class="block w-full overflow-visible text-[clamp(3rem,6vw,6rem)] font-bold uppercase leading-none"
    ...
    >
      ...
      <text v-if="ready" xml:space="preserve" class="fill-white">

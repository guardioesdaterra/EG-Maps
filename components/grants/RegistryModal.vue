        ...
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <div v-for="grant in grants" :key="String(grant.id)" class="rounded border border-white/10 bg-white/5 p-3 text-white">
            <div class="flex items-start justify-between gap-2">
              ...
              <span v-if="grant.relevant" class="text-[10px] font-bold uppercase tracking-wider bg-white/10 px-2 py-0.5 rounded">{{ t('grantsPortal.public') }}</span>
            </div>
            ...
            <div class="mt-3 flex items-center justify-between text-[11px] text-white/60">
              ...
              <span>{{ grant.created_at ? new Date(grant.created_at).toLocaleDateString() : '' }}</span>
            </div>
...
defineProps<{
  show: boolean
  loading: boolean
  grants: Array<GrantRecord & { relevant?: boolean }>
}>()
